...
describe('tiers', () => {
  ...
  it('shows untagged items everywhere and advanced-tier items only in Advanced', async () => {
    const { shownInMode, $showsAdvancedChrome, setInterfaceMode } = await loadStore()
    const items = [{ id: 'settings' }, { id: 'hud', tier: 'advanced' as const }]

    expect(items.filter(shownInMode('advanced')).map(item => item.id)).toEqual(['settings', 'hud'])
    expect(items.filter(shownInMode('simple')).map(item => item.id)).toEqual(['settings'])
