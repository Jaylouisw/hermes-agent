...
// panes, technical tool payloads, the capabilities/cron/artifacts rows) rests
...
export const $interfaceMode = persistentAtom<InterfaceMode>(INTERFACE_MODE_STORAGE_KEY, DEFAULT_INTERFACE_MODE, modeCodec)
...
export const isInterfaceMode = (value: unknown): value is InterfaceMode =>
  INTERFACE_MODES.includes(value as InterfaceMode)
...
function resolvePolicy<K extends ModePolicyKey>(key: K, mode: InterfaceMode, context: ModeContext): ModePolicy[K] | undefined {
...
/** An item with no tier shows in every mode. */
export type InterfaceTier = 'advanced'

const TIER_MODES: Record<InterfaceTier, readonly InterfaceMode[]> = {
  advanced: ['advanced']
}
...
export const $showsAdvancedChrome: ReadableAtom<boolean> = computed($interfaceMode, mode =>
  TIER_MODES.advanced.includes(mode)
)
